...
/**
 * 视图示意图。画各自的样子而不配图标 —— 「点位」与「区县」都是抽象词，
 * 缩略图让人直接认出切过去会看到什么。
 *
 * 图形用 currentColor 随选中态变化，不必另配一套颜色。
 */
...
/**
 * 足迹视图切换。两种视图互斥 —— 叠在一起时区县色块会与点标记争夺注意力。
 * 放地图左下而非侧栏：切换的是地图本身，属取景操作；右下让给缩放控件。
 */
export function ViewSwitch({ value, onChange, className }: ViewSwitchProps) {
  return (
    <div
      ...
      aria-label="足迹视图"
      /**
       * 面板：磨砂玻璃浮在地图之上。不能用 raised 配 line —— 暗色下 0.185 的底
       * 压在 0.145 的地图上只差 0.04，描边 0.3 也淡，控件会像贴在地图里的同色块。
       *
       * 三处一起给层次：底色留透明并模糊背后地图（景深比实色更能说明"在上面"）、
       * 描边换 line-strong（暗色 0.48，对地图有 3 倍以上明度差）、再加投影托底。
       */
      className={clsx(
