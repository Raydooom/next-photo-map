...
/**
 * 宽屏右侧为城市面板预留的宽度：面板宽 260、离右边缘 16，再留出城市标记的
 * 半径与余量 —— 否则边界框虽落在留白内，标记仍会有一角探进面板。
 */
...
/**
 * 取景留白。宽屏右侧预留城市面板的宽度，窄屏的城市选择在地图下方故无需预留。
 *
 * 依据视口宽度而非地图容器宽度：面板显隐由 CSS 的 md 断点控制，而容器两侧还有
 * 页面留白、宽度总小于视口，按容器判断会在临界区间错判成窄屏 —— 面板已显示、
 * 地图却没给它让位。
 */
