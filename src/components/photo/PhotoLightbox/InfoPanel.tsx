...
/**
 * 主读数：大号等宽数字，下方是图标与极小标签。取相机机顶屏的写法，
 * 用大小对比建立层次而不靠格线。图标与标签同处一行 —— 它是标签的视觉前缀，
 * 单独占一层会把读数区撑成三层、又回到格子的观感。
 */
function Readout({
  icon,
  label,
...
/**
 * 次要细节的流式行：用间隔点串成一句。白平衡、测光这类逐行列出会让它们
 * 占到和主参数同等的视觉重量。
 */
...
/**
 * 拍摄信息面板。版式取自相机机顶屏与器材铭牌而非数据表：曝光四要素大号横排、
 * 机身镜头直接刻型号不配标签、次要参数压成一条读数带、地点以地名领起。
 *
 * 全面板只有一条分隔线（曝光区与地点区之间）—— 分组优先靠留白与字号差，
 * 逐行收线再塞进 2×2 单元格读起来就是一张表格。
 */
export function InfoPanel({ photo, onClose }: InfoPanelProps) {
  /**
   * 直接用列表带下来的数据，不再请求详情 —— 列表接口已带 withLocation /
   * withExif / withAiAnalysis，且与详情走同一个 transformPhoto，结构一致。
   */
